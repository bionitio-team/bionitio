...
This is an R implementation of biotool.
...
biotool.R was tested on R version 3.2.

### Installing dependencies

Biotool requires the optparse and seqinr packages which can be installed by:

```bash
Rscript -e "install.packages(c('optparse', 'seqinr'), repos='http://cran.rstudio.org')"
```

### Installing the biotool package

To install the biotool package:

```bash
R CMD INSTALL biotool
```
...
```
% ./biotool.R -h
Usage: ./biotool.R [OPTIONS] [FASTA_FILE [FASTA_FILE ...]]
Print FASTA stats.
...
```
% ./biotool.R file1.fa file2.fa file3.fa
FILENAME	NUMSEQ	TOTAL	MIN	AVG	MAX
file1.fa	5264	3801855	31	722	53540
...
For testing, biotool requires the devtools and lintr packages which can be installed by:
...
```bash
Rscript -e "devtools::test('biotool')"
```

Or to check the whole package:

```bash
Rscript -e "devtools::check('biotool')"
```
...
```
% ./biotool.R ../test_data/two_sequence.fasta 
